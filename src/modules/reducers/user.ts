...
import { ConsultantInfoType, UserAction, UserType, UserInfo } from 'modules/types/user';
...
const userReducer = createReducer<UserType, UserAction>(initialState, {
  ...
  [types.SORT_USERLIST]: (state, action) => {
    let originUserList = Object.assign([], state.userList.users) as Array<UserInfo>;
    let filterUserList = Object.assign([], state.filterUserList.users) as Array<UserInfo>;
    return produce(state, (draft) => {
      ...
      switch(action.payload) {
        case 0:
          if(state.filterUserList.users.length>0) {
            draft.filterUserList.users=getSortList(filterUserList,"branch_name");
            return;
          }
          // draft.userList.users=originUserList.sort((r1, r2) => collator.compare(r2.branch_name,r1.branch_name)).reverse();
        ...
        case 1:
          if(state.filterUserList.users.length>0) {
            draft.filterUserList.users=getSortList(filterUserList,"team_name");
            return;
          }
          // draft.userList.users=originUserList.sort((r1, r2) => collator.compare(r2.team_name,r1.team_name)).reverse();
        ...
        case 2:
          if(state.filterUserList.users.length>0) {
            draft.filterUserList.users=getSortList(filterUserList,"admin_id").reverse();
            return;
          }
          draft.userList.users=getSortList(originUserList,"admin_id").reverse();
          // draft.userList.users=originUserList.sort((r1, r2) => collator.compare(String(r2.admin_id),String(r1.admin_id)));
          break;
        case 3:
          if(state.filterUserList.users.length>0) {
        ...
        case 4:
          if(state.filterUserList.users.length>0) {
            draft.filterUserList.users=getSortList(filterUserList,"user_name");
            return;
          }
