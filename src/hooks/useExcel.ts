...
import {
  ...
  setInitializeAllCallStatisticsByTeam,
...
function useExcel() {
  ...
  const allCallStatisticsByTeamData = useSelector(
    (state: RootState) => state.statistics.allCallStatisticsByTeam,
  );
  ...
  const setInitAllCallStatisticsByTeam = useCallback(() => {
    dispatch(setInitializeAllCallStatisticsByTeam());
  }, [dispatch]);
  ...
  /**
   * @description 팀별 통화 통계 엑셀
   */
  useEffect(() => {
    if (allCallStatisticsByTeamData.length < 1) {
      return;
    }

    const infoTitleName = tableTitleCallStatisticsByTeam
      .slice(0, 3)
      .map((property) => {
        return property.title;
      });
    ...
    allCallStatisticsByTeamData.forEach((values) => {
      const all = StatisticsFormat.getExcelAllCallStatisticsByTeamItem(
        titleName,
        values,
      );

      const outcoming =
        StatisticsFormat.getExcelOutcomingCallStatisticsByTeamItem(
          titleName,
          values,
        );

      const incoming =
        StatisticsFormat.getExcelIncomingCallStatisticsByTeamItem(
          titleName,
          values,
        );
    ...
    const fileName = `call_statistics_by_team.xlsx`;
    XLSX.writeFile(wb, fileName);

    // 초기화
    setInitAllCallStatisticsByTeam();
  }, [allCallStatisticsByTeamData, setInitAllCallStatisticsByTeam]);
